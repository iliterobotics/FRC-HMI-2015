...
implements IRobotConnector
{
  ...
  {
	  ByteBuffer bytes = ByteBuffer.wrap(pData); //TODO
  }
  ...
  public void writeMsg(Integer type, Integer length, byte[] data) throws Exception
  {
	  if(this.isConnected()) {
		  
		  System.out.println("Writing type = " + type + " (" + Integer.toHexString(type) + ")");
		  //write the type
		  mClientSocket.getOutputStream().write(type.byteValue());
		  
		  //write the length
		  mClientSocket.getOutputStream().write(length.byteValue());
		  
		  //write the data
		  mClientSocket.getOutputStream().write(data);
	  }
  }
